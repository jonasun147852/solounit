...
2. **Slash commands**: `/solounit:graft`, `/solounit:doctor`, `/solounit:wallet`, `/solounit:delivery`, `/solounit:sync` — thin wrappers over the CLI in `../oneco/`.
...
Same as the CLI: everything local; the only network-capable path is the explicit `/solounit:sync` command. The triage hook reads the failure event and local advisory files, nothing else.
